...
FactoryGirl.define do
  factory :game do |f|
    ...
    first_small_blind = denominations.select {|denom| denom >= smallest_denomination}.sample
    game_length = (Random.rand(480)+1)*0.05
    f.chips { Random.rand(100000)+1+(2*first_small_blind) }
    f.game_length game_length
    f.round_length { Random.rand((game_length*60).to_i)+1 }
    f.smallest_denomination smallest_denomination
    f.first_small_blind first_small_blind
    f.players {
      players = []
